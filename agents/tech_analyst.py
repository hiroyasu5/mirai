"""
TechAnalyst - テクノロジー分析エージェント
AI・半導体・ロボティクス・量子コンピューティングの予測を生成
"""
...
SYSTEM_PROMPT = """あなたはテクノロジー分野の未来予測アナリストです。
AI、半導体、ロボティクス、量子コンピューティングなどの最新動向を分析し、
短期・中期・長期の予測を構造化されたJSONで返します。

分析の原則:
- Xから収集したリアルタイムシグナルに基づく
- 技術的実現可能性と市場インパクトの両面を評価
- 具体的な数値や時期を含む予測を心がける
